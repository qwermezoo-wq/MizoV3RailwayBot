...
CAPITAL=10000.0; RISK_PCT=1.0; STOP_MULT=1.5; TGT_MULT=3.0; MAX_OPEN=4
...
def analyze(sym):
    ...
    win=h4[-100:]; cl=[c["close"] for c in win]; hi=[c["high"] for c in win]
    lo=[c["low"] for c in win]; vl=[c["volume"] for c in win]
    e20,e50=ema(cl,20),ema(cl,50); a=atr(hi,lo,cl); rs=rsi(cl); adxv=adx(hi,lo,cl)
    avg_vol=sum(vl[-20:])/20
    if e20<=0 or e50<=0 or a<=0: return None
    if (a/cl[-1]*100)<0.15: return None
    if win[-1]["volume"]<avg_vol*1.5: return None
    if rs<=30 or rs>=70: return None
    if adxv<22: return None
    ...
    stop=c["close"]-a*STOP_MULT if direction=="Long" else c["close"]+a*STOP_MULT
    target=c["close"]+a*TGT_MULT if direction=="Long" else c["close"]-a*TGT_MULT
    return {"dir":direction,"stop":stop,"target":target,"entry":c["close"],"atr":a,"rsi":rs,"adx":adxv,"trend":trend}
...
def open_demo_trades():
    ...
    for sym,dir_ in demos:
        ...
        usdt-=amount
        tg(
            f"{'🟢' if dir_=='Long' else '🔴'} <b>صفقة تجريبية | {dir_} {sym}</b>\n"
            f"━━━━━━━━━━━━━━━━━\n"
...
tg(
    f"🤖 <b>بوت محاكاة MizoV3 يعمل الآن</b>\n"
    f"━━━━━━━━━━━━━━━━━\n"
        ...
        if len(positions)<MAX_OPEN:
            ...
            for sym in SYMBOLS:
                ...
                tg(
                    ...
                    f"🛑 وقف الخسارة: <b>{sig['stop']:.4f} $</b> ({abs(entry-sig['stop'])/entry*100:.2f}%)\n"
                    f"🎯 هدف الربح: <b>{sig['target']:.4f} $</b> ({abs(sig['target']-entry)/entry*100:.2f}%)\n"
                    f"⚖️ R:R = 1:{rr}\n"
